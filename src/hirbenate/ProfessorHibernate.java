...
public class ProfessorHibernate implements ProfessorDAO{
	...
	@Override
	public void insert(Professor o) {
	...
	@Override
	public void update(Professor o) {
		// TODO Auto-generated method stub
		Session session = HibernateUtil.getSession();
        try {
            session.beginTransaction();
	...
	@Override
	public void delete(Professor o) {
		// TODO Auto-generated method stub
		Session session = HibernateUtil.getSession();
		try {
			session.beginTransaction();
	...
	public Professor read(Integer id) {
		Session session = HibernateUtil.getSession();
		try {
			return (Professor) session.get(Professor.class.getName(), id);
		} catch (Exception e) {
			session.getTransaction().rollback();
	...
	@Override
	public List<Professor> recuperarPorNome(String nome) {
		return null;
	}
	...
	public List<Professor> recuperarTodos() {
		...
		try {
			session.beginTransaction();
			List<Professor> lista = session.createQuery("from " + Professor.class.getName()).list();
			session.getTransaction().commit();
			return lista;
		} catch (Exception e) {
			session.getTransaction().rollback();
